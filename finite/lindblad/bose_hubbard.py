...
#    Nsteps = 100000
    ...
    for steps in range(Nsteps):
        ...
        list_ene1 = np.array([np.trace(list_rho1[i].dot(list_ham[i])) for i in range(L)])
        print(dt*steps,np.abs(np.trace(list_rho1[0])),np.abs(list_phi1[0]),np.abs(list_ene1[0]))
        list_rho = list_rho1
        list_phi = list_phi1
        list_ham = make_list_ham(op_id,op_a,op_adag,op_n,op_n2,list_J,list_U,list_mu,list_phi)
